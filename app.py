...
@app.callback(  ## @ is a decorator: this function will be used 
...
def update_figure(selected_year):
    ...
    return {
        ...
        'layout': go.Layout(
            ...
            margin={'l': 40, 'b': 40, 't': 10, 'r': 10},    #frame the graph into these margins
            legend={'x': 0, 'y': 1},
            hovermode='closest'
        )
    }
    ...
    html.Div([
        ...
        html.Div([
            dcc.Dropdown(
                ...
                value='Fertility rate, total (births per woman)'  # start value
            ),
            dcc.RadioItems(
                id='xaxis-type',
...
# Chain of outputs and input together
...
# Task 2 The other graph will be a line chart with two DropDown boxes, 
# one for the country and the other for selecting one of the indicators. (hint use Scatter object using mode = 'lines'
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            dcc.Dropdown(   # create drop down menu
                id = 'xaxis-column',
                options = [{'label': i, 'value': i} for i in available_countries],  # do this for all available indicators
        ...
        ],style={'width': '48%', 'float': 'left', 'display': 'inline-block'}) #this takes the other size, if you take 100% the div will just be place down
    ]),
    
    dcc.Graph(id='Country-indicator-graph')
],style={'width': '48%', 'float': 'left', 'display': 'inline-block'})
    
@app.callback(   
    dash.dependencies.Output('Country-indicator-graph', 'figure'),
    [dash.dependencies.Input('xaxis-column', 'value'),
     dash.dependencies.Input('yaxis-column', 'value')])
...
def update_graph(xaxis_column_name, yaxis_column_name, year_value):
    ...
    return {
        'data': [go.Scatter(
            x=dff[dff[''] == xaxis_column_name]['Time'],
            y=dff[dff['Indicator'] == yaxis_column_name]['Value'],
            text=dff[dff['Indicator'] == yaxis_column_name]['Country'],
            mode='markers',
            marker={
